...
#define tassert1(COND) if (!(COND)) { fprintf (stderr, "assert %s failed: (%s:%d)\n", #COND, __FILE__, __LINE__); ::abort(); }

/// Checks if a condition is not true, and returns 1 in that case.
#define tcheck(COND,MSG) if (!(COND)) { fprintf (stderr, "%s failed: %s (%s:%d)\n", #COND, MSG, __FILE__, __LINE__); return 1; }
/// Like tcheck, without msg
#define tcheck1(COND) if (!(COND)) { fprintf (stderr, "%s failed (%s:%d) \n", #COND, __FILE__, __LINE__); return 1; }
