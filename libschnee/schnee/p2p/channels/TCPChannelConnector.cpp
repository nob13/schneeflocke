...
void TCPChannelConnector::startConnecting_locked (CreateChannelOp * op) {
	int timeMs = op->lastingTimeMs(0.5f);
	op->setState (CreateChannelOp::AwaitTcpInfo);
	Error result = mProtocol.requestDetails (op->target, aOpMemFun (op, &TCPChannelConnector::onTcpConnectResult_locked), timeMs);
	if (result) {
...
void TCPChannelConnector::onTcpConnectResult_locked (CreateChannelOp * op, Error result, const ConnectDetails& details) {
	if (result){
		// error, kill op
...
void TCPChannelConnector::connectNext_locked (CreateChannelOp * op){
...
void TCPChannelConnector::onConnect_locked (CreateChannelOp * op, Error result) {
	if (result)
		connectNext_locked (op);
	else {
		// Try to encrypt it
		op->setState (CreateChannelOp::TlsHandshaking);
		op->tlsChannel = TLSChannelPtr (new TLSChannel (op->socket));
		Error e = op->tlsChannel->clientHandshake(TLSChannel::DH, aOpMemFun (op, &TCPChannelConnector::onTlsHandshake_locked));
		if (e) {
...
void TCPChannelConnector::onTlsHandshake_locked (CreateChannelOp * op, Error result) {
	if (result) {
...
void TCPChannelConnector::onNewConnection (){
	...
	op->socket = socket;
	op->tlsChannel = TLSChannelPtr (new TLSChannel (op->socket));
	Error e = op->tlsChannel->serverHandshake(TLSChannel::DH, aOpMemFun (op, &TCPChannelConnector::onAcceptTlsHandshake_locked));
	if (e) {
...
void TCPChannelConnector::onAcceptTlsHandshake_locked (AcceptConnectionOp * op, Error result) {
	if (result) {
